...
/*
 * 查询外卖员评分大于等于8分的外卖员微信号
 * Select emp.employee_wechat
 * from Employee emp, Evaluation eva
 * where emp.employee_id = eva.employee_id and eva.employee_grade >= 8 ;
 */
public class Query2 {
...
//        Data_creation.create_Evaluation(inputPath2);
        ...
        System.out.println("Query2 Block nest loop join 耗时=" + (endTime - startTime) + "ms");
        if (ans.size() != 0) {
        ...
        System.out.println("Query2 indexB+ join 耗时=" + (endTime2 - startTime2) + "ms");
        if (ans.size() != 0) {
        ...
        System.out.println("Query2 Merge join 耗时=" + (endTime3 - startTime3) + "ms");
        if (ans.size() != 0) {
        ...
        while (ReadTable.flag == 0) {
            ...
            for (String tuple : HashTuples) {
                if (tuple == null) break;
                String[] temp = tuple.split("\\s+");
                if (Integer.parseInt(temp[11]) >= 8) { //从join的结果中筛选评价大于8的
                    ans.add(tuple);
                }
            }
        }
        ...
        System.out.println("Query2 Hash join 耗时 = " + (endTime4 - startTime4) + "ms");
        if (ans.size() != 0) {
